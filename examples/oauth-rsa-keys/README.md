...
**Не программист?** Встраиваете авторизацию в свой плагин с помощью нейросети — начните с **[INTEGRATION_CHECKLIST.md](./INTEGRATION_CHECKLIST.md)**: чеклист из 7 обязательных мест в коде, готовый промпт для AI и разбор отладочных логов. Эталон кода — этот же пример (`src/`).
...
## Интеграция в свой плагин (чеклист и отладка)

Полное руководство для не-программистов и нейросетей: **[INTEGRATION_CHECKLIST.md](./INTEGRATION_CHECKLIST.md)**

Кратко — что чаще всего ломается:

1. **Префикс в двух местах** — `productConfig.installationIdPrefix` и сборка `installationId` в `appInfo.ts` должны давать один и тот же prefix (см. эталон `src/appInfo.ts`).
2. **Два имени приложения** — `client_id` (OAuth) и `programName` (лицензия) должны совпадать и относиться к **вашему** продукту.
3. **Сеть в manifest** — нужны и `retouch4.me`, и `3dlutcreator.com`.
4. **Продукт на сервере** — без регистрации лицензия не выдаётся (можно временно протестировать на Vectorscope — см. чеклист).

После интеграции добавьте логи `[LICENSE]` и сверьте `hashPayload` и `isValid: true` — пример ожидаемого вывода в чеклисте.
